...
Augmented Dickey–Fuller tests (reported in the appendix) cannot reject a unit root in either log GDP or log NTL at conventional levels, while their first differences are stationary, consistent with both series being I(1). The Engle–Granger residual test on the OLS above rejects the null of no cointegration at the 5 percent level but not at the 1 percent level, and the Johansen procedure rejects rank zero at the 5 percent level. The cointegration evidence is therefore borderline rather than decisive, and we treat the long-run relationship with appropriate caution; the ARDL-in-levels approach we adopt is well-defined under the Pesaran–Shin–Smith bounds framework regardless of whether the series are I(0) or I(1).
...
To assess directly whether nighttime lights carry predictive information about future GDP — the leading-indicator framing implied by our title — we also run pairwise Granger-causality tests between log NTL and log GDP at lags 1 through 4 (full output in the appendix). The results temper the leading-indicator interpretation: Granger evidence that NTL predicts GDP beyond GDP's own lags is, at best, marginal at the conventional 5 percent level, with p-values close to or above the threshold for several lag choices. We therefore frame NTL throughout the paper as a contemporaneous proxy and a candidate input into multi-variable nowcasting, rather than as a stand-alone leading indicator.
...
It is worth emphasising that the DFE result above is not corroborated by the more flexible PMG and MG estimators in the same table. The long-run NTL coefficient is significant under DFE in both the baseline and the scarring specifications, marginal under PMG, and either insignificant or wrong-signed under MG once the scarring dummy is included. To assess whether the long-run homogeneity restriction underlying PMG (and, more strongly, DFE) is supported by the data, we run Hausman tests of PMG against MG for the baseline, +Covid, and +Scarring specifications (output reported in the appendix Stata log). The point of the comparison is that DFE is the most efficient of the three but only consistent if the long-run coefficient is common across all 34 provinces; MG is consistent under heterogeneity but loses efficiency. The Indonesian provincial economy is plainly heterogeneous — Java's services-and-manufacturing base, Kalimantan and Papua's mining-driven output, and Sulawesi's largely agricultural structure each generate very different NTL–GDP elasticities — so we are inclined to read a rejection of homogeneity as substantively meaningful rather than a statistical artefact. We therefore present DFE as one specification among several rather than as a preferred estimator and treat the cross-estimator disagreement as itself an informative finding about provincial heterogeneity.

# Conclusion

This paper has used Indonesian quarterly real GDP and Black Marble nighttime light radiance from 2012 to 2024 to ask, in a deliberately stripped-down setting, how much economic information about Indonesia can be recovered from satellite light alone. We summarise our findings as a short list of what the analysis does, and does not, support.

**What we find.** Nighttime light is positively and generally significantly correlated with GDP across OLS, ARDL, and provincial panel specifications. Once autocorrelation (ARDL) and provincial heterogeneity (FE/TWFE) are controlled for, the OLS elasticity of about 0.56 falls by an order of magnitude to roughly 0.05 in the within-province dimension, indicating that most of the raw OLS association is between-province rather than time-series identification. There is a clear structural break in the GDP series around 2020, and adding a post-2020 dummy substantially improves both the in-sample fit and the out-of-sample forecast accuracy of the ARDL.

**What we do not find.** We do not find evidence that nighttime light is a strong stand-alone predictor of GDP. In the preferred ARDL+Scar specification the contemporaneous NTL coefficient is significant only at the 10 percent level, and the bulk of the forecast performance comes from lagged GDP and the post-2020 dummy. We also do not find clean Granger-causal evidence that NTL leads GDP at conventional significance levels, which qualifies the leading-indicator framing implied by our title. We are unable to causally attribute the post-2020 break to COVID specifically — the scarring dummy will absorb any persistent post-2020 shift, including BPS methodology changes, commodity-price re-pricing, or other contemporaneous shocks — and we therefore decline to interpret the −2 percent coefficient as a precise estimate of pandemic damage. Finally, the disagreement across DFE, PMG, and MG estimators in the provincial panel suggests that the long-run NTL–GDP relationship is heterogeneous across provinces; pooled estimates that look significant under DFE may not survive when heterogeneity is allowed.

There are two potential key takeaways from the national-level analysis. First, accounting for a post-2020 regime shift is critical for forecasting GDP, more than it might be for other indicators. We can see this especially when we use quarterly dummies. From @fig-4, The panel (c) and (f) show that quarterly dummies do not significantly add insight, while panel (d) and (e), panels with quarterly dummies without scarring, show a looser fit. This pattern is consistent with other studies that discuss a potential scarring effect from the COVID pandemic in Indonesia [@pangestu], though we caution that our scarring dummy captures any persistent post-2020 shift and does not, on its own, identify a specific channel or a precise long-run magnitude.
...
This NTL–GDP divergence during COVID is more than a curiosity, because it cuts against one of the original motivations for using nighttime lights in the first place. NTL is appealing as an *independent* validation tool precisely because it is generated outside the statistical agency and is therefore immune to revisions or measurement choices. But that independence is most valuable in periods when official numbers are hardest to interpret — sharp downturns, structural breaks, regime changes — and it is exactly during such episodes (here, the 2020 trough) that NTL departs most visibly from GDP. Whether the gap is driven by cloud cover and gap-filling, by electricity subsidies that decouple lighting from output, by the resilience of household-level lighting during commercial slowdowns, or by some combination of these channels, we cannot tell from the present data; both explanations above are plausible but neither is tested here. The implication is that NTL should not be treated as a counterweight to official GDP when the two diverge most, and crisis episodes call for additional independent evidence (e.g., electricity dispatch, mobility, fiscal flows) rather than reliance on NTL alone.

The regional level analysis is broadly consistent with the national-level findings. A pattern suggestive of hysteresis — what we label as a scarring effect — also appears in the provincial panel. The DFE baseline yields a speed of convergence in line with the literature, and adding the scarring dummy slows it further. We emphasize, however, that this is an associational pattern rather than a causally identified estimate: the dummy captures any persistent post-2020 level shift, and the implied adjustment dynamics should be interpreted with the same caution we flag at the national level. Additionally, the nightlight index is also not as important as we might think, as its short-term dynamics show weak significance.
...
The exercise above is constrained in several ways that future work could relax.

First, the **national sample is small** (51 quarterly observations from 2012Q1 to 2024Q4), and the more heavily parameterised ARDL specifications — the +Scar variant in particular — sit close to the limits of what the degrees of freedom support. Standard errors and lag-selection decisions should be read with this in mind.

Second, the **out-of-sample forecast window is short**. Our train/test split holds out only the four to six quarters of 2024 onward, which is too few observations to make strong claims about forecast accuracy. A rolling-origin or recursive evaluation across multiple cuts would give a more honest picture of forecast performance and is a natural next step.

Third, the **identification of the scarring effect is fragile**. The post-2020 dummy captures any persistent shift after that date, not specifically the pandemic. We did not run a formal structural-break test (e.g., Bai–Perron, Quandt–Andrews) to locate the break date, nor did we re-parameterise the dummy to start at later dates (2022Q1, 2023Q1) as a sensitivity check. The +Scar ARDL specification also exhibits unusual autoregressive dynamics — the sum of AR coefficients flips from roughly +0.85 in the baseline to roughly −2.5 in the +Scar specification — which suggests the scarring dummy is partly absorbing variation that the AR structure should be carrying. An error-correction (ECM) reformulation of the ARDL+Scar would identify the long-run scarring coefficient from the cointegrating relationship rather than from a saturated AR polynomial, and we leave this for the next revision.

Fourth, we report **no formal cointegration bounds test**. Pesaran–Shin–Smith bounds testing is the standard companion to ARDL-in-levels and would substantially strengthen the long-run interpretation; we plan to add it in Stata using the `ardl` package alongside the migrated provincial pipeline.

Fifth, the **provincial models do not currently allow heterogeneity beyond the intercept**. A natural extension is to interact NTL with province characteristics (sectoral structure, urbanisation, share of mining or agriculture in GDRP) or to estimate the NTL–GDRP elasticity cluster-wise across the seven major island groups, both of which would let us speak directly to the heterogeneity that the cross-estimator disagreement above already hints at.

Finally, the COVID-period **NTL–GDP divergence** itself deserves more direct investigation than we provide here. The candidate explanations we offer — cloud cover and gap-filling, electricity subsidies, household versus commercial lighting — are plausible but untested. Pairing NTL with electricity dispatch data, mobility indices, or fiscal flow records during 2020–2021 would let future work decompose the divergence and recover at least some of the independent-validation value that NTL is supposed to provide.
